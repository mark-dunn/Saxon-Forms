...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="xforms:*" mode="get-context-instance-id" as="xs:string">
        ...
        <xsl:variable name="referenced-instance-id" as="xs:string?" select="if ($nodeset != '') then xforms:getInstanceId($nodeset) else ()"/>
    ...
    <xsl:template match="xforms:*" mode="get-properties">
        ...
        <xsl:variable name="iterate-ref" as="xs:string?">
            <xsl:choose>
                <xsl:when test="exists(@iterate)">
                    <xsl:apply-templates select="." mode="get-data-iterate-ref"/>
                </xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
        </xsl:variable>
                ...
                <xsl:when test="$refi != '' and exists($bindings-this-instance)">
                    ...
                    <xsl:for-each select="$bindings-this-instance">
                        ...
                        <xsl:variable name="context-node" as="node()*">
                            <xsl:try>
                                <xsl:evaluate xpath="$nodeset-mod" context-item="$instanceXML" namespace-context="$default-namespace-context"/>
                                <xsl:catch>
                                    ...
                                    <xsl:evaluate xpath="xforms:impose($nodeset-mod)" context-item="$instanceDoc" namespace-context="$default-namespace-context"/> 
                                </xsl:catch>
                            </xsl:try>
                        </xsl:variable>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:variable name="binding-context-node" as="node()*">
                                    <xsl:try>
                                        <xsl:evaluate xpath="$binding-nodeset-mod" context-item="$instanceXML" namespace-context="$default-namespace-context"/>
                                        <xsl:catch>
                                            <xsl:variable name="instanceDoc" as="document-node()">
                                                <xsl:document>
                                                    <xsl:sequence select="$instanceXML"/>
                                            ...
                                            <xsl:evaluate xpath="$binding-nodeset-mod" context-item="$instanceDoc" namespace-context="$default-namespace-context"/> 
                                        </xsl:catch>
                                    </xsl:try>
                                </xsl:variable>
        ...
        <xsl:map>
            ...
            <xsl:if test="exists($iterate-ref)">
                <xsl:map-entry key="'iterate'" select="$iterate-ref"/>
            </xsl:if>
        </xsl:map>
    ...
    <xsl:template match="xforms:*" mode="get-data-ref" as="xs:string">
        ...
        <xsl:variable name="ref" as="xs:string" select="if (exists(@ref) or exists(@nodeset)) then xs:string((@ref,@nodeset)[1]) else ''"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="this-context" as="xs:string?" select="
                    ...
                    else if ($nodeset != '') then '.'
                    else 'instance(''' || $default-instance-id || ''')'"/>
                
                <xsl:variable name="this-iterate-context" as="xs:string?" select="
                    if ( exists(@iterate) )
                    then  normalize-space( xs:string(@iterate) )
                    else ()"/>
                
                <xsl:variable name="resolved-context" select="xforms:resolveXPathStrings($nodeset,$this-context)"/>
                
                <xsl:variable name="resolved-iterate-context" as="xs:string" select="if ($this-iterate-context) then xforms:resolveXPathStrings($this-context,$this-iterate-context) else $resolved-context"/>
                ...
                <xsl:variable name="data-ref" as="xs:string">
                    <xsl:choose>
                        <xsl:when test="exists($this-ref)">
                            <!--<xsl:message use-when="$debugMode"><xsl:sequence select="$log-label"/> resolving $this-ref in context $nodeset = <xsl:sequence select="$resolved-iterate-context"/></xsl:message>-->
                            <xsl:sequence select="xforms:resolveXPathStrings($resolved-iterate-context,$this-ref)"/>
                        </xsl:when>
    ...
    <xd:doc scope="component">
        <xd:desc>
            <xd:p>get-data-iterate-ref mode: return XPath binding expression relevant to context XForm element</xd:p>
        </xd:desc>
        <xd:param name="nodeset">An XPath binding expression. If it exists, $this/@iterate is evaluated relative to it.</xd:param>
        <xd:param name="default-instance-id">ID of default instance in XForm.</xd:param>
    </xd:doc>
    <xsl:template match="xforms:*" mode="get-data-iterate-ref" as="xs:string">
        <xsl:param name="nodeset" as="xs:string" select="''" tunnel="yes"/>
        <xsl:param name="default-instance-id" as="xs:string" required="no" select="$global-default-instance-id" tunnel="yes"/>
        
        <xsl:variable name="log-label" as="xs:string" select="concat('[get-data-iterate-ref mode for ', name(), ']')"/>
        <xsl:message use-when="$debugMode"><xsl:sequence select="$log-label"/> START</xsl:message>
        <xsl:variable name="time-id" select="concat($log-label, ' ', generate-id())"/>
        <xsl:sequence use-when="$debugTiming" select="js:startTime($time-id)" />    
        
        <xsl:variable name="iterate-ref" as="xs:string" select="xs:string(@iterate)"/>
        ...
        <xsl:choose>
            <xsl:when test="exists(@bind)">
                ...
                <xsl:choose>
                    <xsl:when test="exists($this-binding)">
                        <xsl:apply-templates select="$this-binding" mode="get-data-ref">
                            <xsl:with-param name="nodeset" select="$iterate-ref"/>
                        </xsl:apply-templates>
                    </xsl:when>
                    <xsl:otherwise>             
                        <xsl:call-template name="xforms-binding-exception">
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="resolved-iterate-context" as="xs:string" select="xforms:resolveXPathStrings($this-context,$iterate-ref)"/>
                
                <!--<xsl:message use-when="$debugMode"><xsl:sequence select="$log-label"/> $iterate-ref = <xsl:sequence select="$iterate-ref"/></xsl:message>
                <xsl:message use-when="$debugMode"><xsl:sequence select="$log-label"/> $this-context = <xsl:sequence select="$this-context"/></xsl:message>-->
                
                
                <xsl:sequence select="$resolved-iterate-context"/>
            </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:variable name="action-map" as="map(*)">
            <xsl:map>
                ...
                <xsl:if test="exists(@ref) or exists(@nodeset)">
                    <xsl:map-entry key="'@orig-ref'" select="normalize-space( xs:string((@ref|@nodeset)[1]) )"/>
                </xsl:if>
            ...
            -->
                ...
                <xsl:if test="exists($switch-id)">
                    <xsl:map-entry key="'@switch-id'" select="$switch-id"/>
                </xsl:if>
...
<!--                         <xsl:message use-when="$debugMode">[HTTPsubmit] response body: <xsl:value-of select="serialize($response)"/></xsl:message>-->
                      ...
                      <xsl:when test="$replace = 'text' and exists($targetref) and $response castable as xs:string">
                          ...
                          <xsl:variable name="updatedNode" as="node()">
                              <xsl:try>
                                  <xsl:evaluate xpath="xforms:impose($targetref)" context-item="$instanceXML" namespace-context="$instanceXML"/>
                                  <xsl:catch>
                                      <xsl:evaluate xpath="xforms:impose($targetref)" context-item="$instanceDoc" namespace-context="$instanceDoc"/>
                                  </xsl:catch>
                              </xsl:try>
                          </xsl:variable>
    ...
    <xsl:template match="/">
        <xsl:message use-when="$debugMode">[document node] Root element is <xsl:sequence select="name(*)"/></xsl:message>
        ...
        <xsl:variable name="instanceField" as="node()?">
            <xsl:choose>
                ...
                <xsl:when test="starts-with(@value,'instance(')">
                    <xsl:sequence select="js:getInstance(xforms:getInstanceId(string(@value)))"/>
                </xsl:when>
                <xsl:when test="$nodeset != ''">
        ...
        <xsl:variable name="valueExecuted" as="xs:string">
            <xsl:choose>
                <xsl:when test="exists($binding/@calculate)">
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose('string(' || $binding/@calculate || ')')" context-item="$instanceField" namespace-context="$namespace-context-item"/> 
                        <xsl:catch>
                            <xsl:evaluate xpath="xforms:impose('string(' || $binding/@calculate || ')')" context-item="$instanceDoc" namespace-context="$instanceDoc"/> 
                        </xsl:catch>
                    </xsl:try>
                </xsl:when>
                <xsl:when test="exists(@value)">
                    ...
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose('string(' || @value || ')')" context-item="$instanceField" namespace-context="$namespace-context-item"/> 
                        <xsl:catch>
                            <xsl:evaluate xpath="xforms:impose('string(' || @value || ')')" context-item="$instanceDoc" namespace-context="$instanceDoc"/> 
                        </xsl:catch>
                    </xsl:try>
                </xsl:when>
                <xsl:otherwise>
...
<!--            <xsl:attribute name="class" select="$htmlClass"/>-->
    ...
    <xsl:template match="xforms:label">
        <xsl:param name="nodeset" as="xs:string" select="''" tunnel="yes"/>
        <xsl:param name="instance-context" as="xs:string" select="''" tunnel="yes"/>
        ...
        <xsl:variable name="label" as="item()*">
            <xsl:choose>
                <xsl:when test="exists(@bind) and exists($bindingi/@calculate)">
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose('string(' || $bindingi/@calculate || ')')" context-item="$instanceField" namespace-context="$namespace-context-item"/>              
                        <xsl:catch>
                            <xsl:evaluate xpath="xforms:impose('string(' || $bindingi/@calculate || ')')" context-item="$instanceDoc" namespace-context="$instanceDoc"/>              
                        </xsl:catch>
                    </xsl:try>
                </xsl:when>
                <xsl:when test="exists(@ref)">
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose($refi)" context-item="$instanceField" namespace-context="$namespace-context-item"/>              
                        <xsl:catch>
...
<!--        <xsl:message use-when="$debugMode">[recalculate mode] comparing instance node <xsl:sequence select="fn:serialize(.)"/> with updated node <xsl:sequence select="fn:serialize($updated-nodes)"/></xsl:message> -->
    ...
    <xsl:function name="xforms:addNamespaceDeclarationsToDocument" as="document-node()">
        ...
        <xsl:document>
            <xsl:variable name="default-namespace" as="xs:anyURI" select="$this/*/namespace-uri()"/>
            <!--<xsl:message use-when="$debugMode">Default namespace of root <xsl:sequence select="name($this/*)"/>: <xsl:sequence select="$default-namespace"/></xsl:message>-->
            <xsl:element name="{name($this/*)}" namespace="{$default-namespace}">
                <xsl:namespace name="xforms" select="'http://www.w3.org/2002/xforms'"/>
                <xsl:for-each select="$this//*[not(namespace-uri() = ('','http://www.w3.org/2002/xforms',$default-namespace))][not(namespace-uri() = (ancestor::*/namespace-uri(),preceding::*/namespace-uri()))]">
                    <xsl:variable name="new-namespace" select="namespace-uri(.)"/>
                    ...
                    <xsl:if test="exists($new-prefix)">
                        <!--<xsl:message use-when="$debugMode">Adding namespace to root <xsl:sequence select="name($this/*)"/>: <xsl:sequence select="$new-prefix"/>=<xsl:sequence select="$new-namespace"/></xsl:message>-->
                        <xsl:namespace name="{$new-prefix}" select="$new-namespace"/>
                    </xsl:if>
                </xsl:for-each>
                <xsl:sequence select="$this/*/@*,$this/*/node()"/>
...
<!--                    <xsl:sequence select="if($isRelevant) then 'relevant' else 'irrelevant'"/>-->
    ...
    <xsl:template name="applyActions">
        ...
        <xsl:variable name="ifExecuted" as="xs:boolean">
            <xsl:choose>
                <xsl:when test="exists($ifVar) and exists($context-node)">
                    ...
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose($ifVar)" context-item="$context-node" namespace-context="$instanceXML2"/>
                        <xsl:catch>
        ...
        <xsl:variable name="isWhileTrue" as="xs:boolean">
            <xsl:choose>
                <xsl:when test="exists($whileVar) and exists($context-node)">
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose($whileVar)" context-item="$context-node" namespace-context="$instanceXML2"/>
                        <xsl:catch>
    ...
    <xd:doc scope="component">
        <xd:desc>
            <xd:p>registerOutput: create map of an xforms:output (or xforms:input) whose value may be updated e.g. during an xforms-refresh. This element ios the context node.</xd:p>
        </xd:desc>
        <xd:param name="id">ID of HTML element.</xd:param>
        <xd:param name="nodeset">XPath binding expression</xd:param>
        <xd:param name="instance-context">ID of XForms instance relevant to this control</xd:param>
        ...
        <xsl:variable name="updated-nodeset" as="node()*">
            <xsl:choose>
                <xsl:when test="exists($iterate-ref)">
                    <xsl:message use-when="$debugMode">[action-setvalue-inner] Applying action @iterate '<xsl:sequence select="$iterate-ref"/>'</xsl:message>
                    
                    <xsl:variable name="ref-local" as="xs:string?" select="map:get($action-map,'@orig-ref')"/>
                    ...
                    <xsl:when test="exists($updated-node)">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:message use-when="$debugMode">[action-setvalue-inner] updated instance (last iteration) <xsl:sequence select="serialize($updatedInstanceXML)"/></xsl:message>
                            </xsl:otherwise>
                        </xsl:choose>
...
<!--        <xsl:message use-when="$debugMode">[action-setindex] $action-map = <xsl:value-of select="serialize($action-map)"/></xsl:message>-->
    ...
    <xsl:template name="action-toggle">
        ...
        <xsl:variable name="switch-id" as="xs:string?" select="map:get($action-map,'@switch-id')"/>
        ...
        <xsl:variable name="case-id" as="xs:string">
            <xsl:choose>
                <xsl:when test="exists($case-id-evaluated) and exists($context-node)">
                    <xsl:try>
                        <xsl:evaluate xpath="xforms:impose($case-id-evaluated)" context-item="$context-node" namespace-context="$context-node"/>
                        <xsl:catch>
                            <xsl:evaluate xpath="xforms:impose($case-id-evaluated)" context-item="$instanceDoc" namespace-context="$instanceDoc"/>    
                        </xsl:catch>
                    </xsl:try>
                </xsl:when>
